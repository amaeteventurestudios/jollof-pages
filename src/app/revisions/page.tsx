...
export default function RevisionsPage() {
  ...
  const handleSendToReview = () => toast("Revision impact sent to Review Queue.", "info");
        ...
        {confirmed && (
          <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-4 mb-5 flex flex-col sm:flex-row items-center justify-center gap-3 text-center max-w-2xl mx-auto">
            ...
            <div>
              ...
              <p className="text-xs text-green-400/70">Downstream staleness markers applied.</p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-5">
            ...
            <div className="jollof-card">
              ...
              <div className="p-4 space-y-3">
                ...
                <div className="space-y-2">
                  <div className="bg-jollof-surface border border-red-500/20 rounded-lg p-2.5">
                  ...
                  <div className="bg-jollof-surface border border-green-500/20 rounded-lg p-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-jollof-subtext mt-0.5">{revision.triggeredBy}</p>
                </div>
          ...
          <div className="lg:col-span-2 space-y-4 w-full max-w-3xl mx-auto lg:max-w-none">
                ...
                {revision.impactedScenes.map((scene) => (
                  <div key={scene.id} className="px-4 py-3">
                    <div className="flex flex-wrap items-center gap-2 mb-0.5">
                      ...
                      <Link href="/scenes/scene-03" className="text-sm font-medium text-jollof-text hover:text-jollof-orange transition-colors">{scene.title}</Link>
                      <StatusBadge status={scene.severity} className="text-[9px]" />
                    </div>
              ...
              <div className="jollof-card">
                ...
                <div className="divide-y divide-jollof-border">
                  {revision.impactedPages.map((page) => (
                    <div key={page.id} className="px-4 py-3 flex items-center justify-between min-h-[52px]">
              ...
              <div className="jollof-card">
                ...
                <div className="divide-y divide-jollof-border">
                  {revision.impactedPanels.map((panel) => (
                    <div key={panel.id} className="px-4 py-3 min-h-[52px]">
                ...
                {revision.impactedCanon.map((canon) => (
                  <div key={canon.id} className="px-4 py-3">
                    <div className="flex flex-wrap items-center gap-2 mb-0.5">
                      ...
                      <Link href="/canon" className="text-sm font-medium text-jollof-text hover:text-jollof-orange transition-colors">{canon.title}</Link>
                      <StatusBadge status={canon.severity} className="text-[9px]" />
                    </div>
            ...
            <div className="jollof-card p-4 text-center">
              ...
              <div className="flex flex-wrap items-center justify-center gap-1.5 text-xs mb-2">
                {[
                  ...
                  { label: "Scene 03", color: "text-amber-400" },
                  { label: "→", color: "text-jollof-label" },
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 justify-center">
              ...
              <Button variant="secondary" icon={Flag} onClick={handleSendToReview} className="flex-1 sm:flex-none justify-center">
                Send to Review Queue
              </Button>
              <Link href="/series" className="flex-1 sm:flex-none">
                <Button variant="ghost" icon={RotateCcw} className="w-full justify-center">Cancel</Button>
              </Link>
            </div>
          </div>
        </div>
      ...
      <Modal open={confirmOpen} onClose={() => setConfirmOpen(false)} title="Confirm Revision" size="sm">
        <div className="space-y-4">
          <div className="bg-amber-500/5 border border-amber-500/20 rounded-lg p-3 flex items-start gap-2">
            ...
            <p className="text-xs text-amber-300">Confirming this revision will mark downstream scenes, pages, and panels as stale. Agents will need to re-validate before re-approval.</p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <Button variant="primary" className="flex-1 justify-center" onClick={handleConfirm}>Confirm Revision</Button>
            <Button variant="secondary" className="justify-center" onClick={() => setConfirmOpen(false)}>Cancel</Button>
          </div>
        </div>
      </Modal>
    </AppShell>
