...
import {
  MOCK_SERIES, MOCK_BOOKS, MOCK_ACTIVITY_FEED
} from "@/lib/mock/jollof-data";
import {
  BookOpen, Shield, ClipboardList, Flag, TrendingUp,
  Zap, Activity, ChevronRight, AlertTriangle, CheckCircle,
  Clock, ExternalLink
} from "lucide-react";
    ...
    <AppShell>
      <div className="p-6 max-w-6xl">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-black text-jollof-text mb-1">Series Command Center</h1>
          <p className="text-sm text-jollof-subtext">See the health of the entire graphic novel series at a glance.</p>
        </div>

        {/* Series header card */}
        <div className="jollof-card p-5 mb-6 flex items-center gap-5">
          {/* Fake cover */}
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-0.5">
              <h2 className="text-xl font-bold text-jollof-text">{MOCK_SERIES.title}</h2>
          ...
          <div className="shrink-0 flex flex-col gap-2">
            <Button variant="outline" icon={BookOpen} size="sm">
              View Series Bible
            </Button>
            <Button variant="secondary" icon={Activity} size="sm" onClick={handleHealthCheck}>
              Run Health Check
            </Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          <MetricCard
            label="Books"
            value={MOCK_SERIES.booksCount}
            sub="3 active"
            icon={BookOpen}
          />
          <MetricCard
            label="Canon Health"
            value={`${MOCK_SERIES.canonHealth}%`}
            sub="2 warnings"
            icon={Shield}
        ...
        <div className="jollof-card p-4 mb-6">
          ...
          <ProgressBar value={MOCK_SERIES.progress} height="h-2" showLabel={false} />
                ...
                {MOCK_BOOKS.map((book) => (
                  <div key={book.id} className="p-4 hover:bg-jollof-panel/40 transition-colors group">
                    <div className="flex items-start gap-3">
                      <div className="w-10 h-12 rounded bg-gradient-to-b from-amber-950 to-jollof-surface border border-jollof-border shrink-0 flex items-center justify-center">
                        <span className="text-[10px] font-bold text-jollof-orange">B{book.issueNumber}</span>
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          <Link href={`/books/${book.id}`} className="text-sm font-medium text-jollof-text hover:text-jollof-orange transition-colors truncate">
                            {book.title}
                          </Link>
                      ...
                      <div className="text-right shrink-0">
                        ...
                        <Link href={`/books/${book.id}`} className="inline-flex items-center gap-1 text-[10px] text-jollof-orange mt-1 hover:underline opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="jollof-card">
              <div className="flex items-center justify-between p-4 border-b border-jollof-border">
                ...
                <span className="text-xs text-jollof-orange hover:underline cursor-pointer">View all</span>
              </div>
              <div className="divide-y divide-jollof-border">
                {MOCK_ACTIVITY_FEED.map((item) => (
                  <div key={item.id} className="px-4 py-2.5 flex items-start gap-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-jollof-label flex items-center gap-1">
              ...
              <div className="p-4 space-y-3">
                {[
                  ...
                  <div key={row.label} className="flex items-center gap-2">
                    <span className="text-xs text-jollof-subtext w-24 shrink-0">{row.label}</span>
                    <ProgressBar value={row.value} className="flex-1" height="h-1" />
                    <span className={`text-[10px] w-12 text-right ${row.status === "healthy" ? "text-green-400" : "text-amber-400"}`}>
                      {row.status === "healthy" ? "Healthy" : "Review"}
                    </span>
                  </div>
                ))}
