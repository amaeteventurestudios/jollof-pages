...
export default function PagePlannerPage() {
  ...
  const handleAutoBalance = () => {
    toast("Auto-balance pacing applied — pages redistributed.", "success");
  };

  const totalPages = 128;
  const assignedPages = assignments.reduce((acc, a) => acc + (a.pageEnd - a.pageStart + 1), 0);

  const estimatedPanels = Math.round(((pageRangeEnd - pageRangeStart + 1) * targetPanels) / (pageRangeEnd - pageRangeStart + 1));
  ...
  return (
    <AppShell>
      <div className="p-5 max-w-7xl">
        ...
        <h1 className="text-2xl font-black text-jollof-text mb-0.5">Page Planner</h1>
        ...
        <div className="jollof-card p-4 mb-5 flex items-center gap-4">
          <div className="flex-1">
            <span className="text-xs text-jollof-label block mb-1">Book</span>
            <select className="bg-jollof-surface border border-jollof-border rounded-lg px-3 py-1.5 text-sm text-jollof-text focus:outline-none focus:border-jollof-orange/40">
              <option>Book 1: The Collapse Conjecture</option>
              <option>Book 2: Drift Zone Uprising</option>
            </select>
          </div>
          <div className="flex items-center gap-2 text-xs text-jollof-subtext">
            <LayoutGrid size={13} className="text-jollof-orange" />
            <span>Total Pages: <strong className="text-jollof-text">{totalPages}</strong></span>
            <span>Assigned: <strong className="text-jollof-text">{assignedPages}</strong></span>
            <span>Remaining: <strong className="text-amber-400">{totalPages - assignedPages}</strong></span>
          </div>
          <div className="flex gap-2 shrink-0">
            <Button variant="secondary" icon={RefreshCw} size="sm" onClick={handleAutoBalance}>Auto-Balance Pacing</Button>
            <Button variant="primary" icon={Save} size="sm" onClick={handleSave}>Save Plan</Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-1">
            <div className="jollof-card">
              <div className="p-4 border-b border-jollof-border">
                ...
                <p className="text-xs text-jollof-label">Total Pages: {totalPages}</p>
              </div>
                  ...
                  <div
                    ...
                    className={`aspect-square rounded text-[8px] font-medium flex items-center justify-center cursor-pointer transition-all hover:scale-110 ${
                      page.status === "approved"
                        ...
                        : "bg-jollof-border text-jollof-label"
                    }`}
                  >
                    {page.number}
                  </div>
              ...
              <div className="p-3 border-t border-jollof-border flex items-center gap-3 text-[10px] text-jollof-label">
          ...
          {/* Scene page assignment table */}
          <div className="lg:col-span-1">
            <div className="jollof-card h-full">
              <div className="p-4 border-b border-jollof-border">
                <h3 className="text-sm font-semibold text-jollof-text">Scene / Page Assignment</h3>
                <p className="text-xs text-jollof-label">Drag scenes or adjust to change page targets.</p>
              </div>
              <div className="divide-y divide-jollof-border">
                {assignments.map((a) => (
                  <div
                    ...
                    className={`px-4 py-3 cursor-pointer hover:bg-jollof-panel/40 transition-colors ${selectedSceneId === a.sceneId ? "bg-jollof-orange/5 border-l-2 border-l-jollof-orange" : ""}`}
                  >
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5 text-[10px] text-jollof-label">
                          <span>pp. {a.pageStart}–{a.pageEnd}</span>
                          <span>{a.pageEnd - a.pageStart + 1} pages</span>
          ...
          <div className="space-y-4">
            ...
            <div className="jollof-card">
              <div className="p-4 border-b border-jollof-border">
                ...
                <p className="text-xs text-jollof-label">
              ...
              <div className="p-4 space-y-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-jollof-surface border border-jollof-border rounded-lg px-3 py-2 text-xs text-jollof-text focus:outline-none focus:border-jollof-orange/40"
                  >
                    {assignments.map((a) => (
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs text-jollof-label mb-1.5">Page Range Start</label>
                    <div className="flex items-center gap-1">
                      <input
                        type="number"
                        value={pageRangeStart}
                        onChange={(e) => setPageRangeStart(Number(e.target.value))}
                        className="flex-1 bg-jollof-surface border border-jollof-border rounded px-2 py-1.5 text-xs text-jollof-text focus:outline-none focus:border-jollof-orange/40 text-center"
                      />
                      <div className="flex flex-col">
                        <button onClick={() => setPageRangeStart((v) => v + 1)} className="text-jollof-label hover:text-jollof-orange"><ChevronUp size={12} /></button>
                        <button onClick={() => setPageRangeStart((v) => Math.max(1, v - 1))} className="text-jollof-label hover:text-jollof-orange"><ChevronDown size={12} /></button>
                      </div>
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs text-jollof-label mb-1.5">Page Range End</label>
                    <div className="flex items-center gap-1">
                      <input
                        type="number"
                        value={pageRangeEnd}
                      ...
                      />
                      <div className="flex flex-col">
                        <button onClick={() => setPageRangeEnd((v) => v + 1)} className="text-jollof-label hover:text-jollof-orange"><ChevronUp size={12} /></button>
                        <button onClick={() => setPageRangeEnd((v) => Math.max(pageRangeStart + 1, v - 1))} className="text-jollof-label hover:text-jollof-orange"><ChevronDown size={12} /></button>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-xs text-jollof-label mb-1.5">Target Panels/Page</label>
                  <input
                    type="number"
                    value={targetPanels}
                    onChange={(e) => setTargetPanels(Number(e.target.value))}
                    className="w-full bg-jollof-surface border border-jollof-border rounded px-3 py-1.5 text-xs text-jollof-text focus:outline-none focus:border-jollof-orange/40"
                  />
                </div>

                <div className="jollof-surface border border-jollof-border rounded-lg p-3 space-y-1.5 text-xs">
                  ...
                  <div className="flex justify-between">
                    <span className="text-jollof-label">Avg panels/page</span>
                    <span className="text-jollof-text">{(estimatedPanels / (pageRangeEnd - pageRangeStart + 1)).toFixed(1)}</span>
                  </div>
                </div>

                <div className="flex gap-2">
                  <Button variant="secondary" size="sm" className="flex-1" onClick={handleApply}>Apply Changes</Button>
                </div>
              </div>
            </div>
                ...
                {[
                  ...
                  <div key={row.label}>
                    <div className="flex items-center justify-between mb-0.5">
                      ...
                      <span className={`text-[10px] ${row.status === "good" ? "text-green-400" : row.status === "warning" ? "text-amber-400" : "text-jollof-label"}`}>
                        {row.status}
                      </span>
                    </div>
                    <ProgressBar value={row.value} height="h-1" />
                  </div>
                ))}
              </div>
        ...
        <div className="grid grid-cols-3 gap-4 mt-5">
          {[
            ...
            { icon: AlertTriangle, title: "Turn Story Structure into Page Structure", desc: "Convert approved scenes directly to pages and build your issue with clarity and control." },
          ].map((c) => (
