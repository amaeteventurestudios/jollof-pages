...
export default function ExportPage() {
  ...
  const treeStatusColor = (status: string | undefined) => {
    if (status === "approved") return "text-green-400";
    if (status === "warning" || status === "draft") return "text-amber-400";
    ...
    <AppShell>
      <div className="px-4 py-5 sm:px-6 sm:py-6 w-full max-w-5xl mx-auto">
          ...
          <div className="flex items-center gap-0 min-w-max w-fit mx-auto">
            {STEPS.map((s, i) => (
              <div key={s} className="flex items-center">
                ...
                {i < STEPS.length - 1 && <ChevronRight size={13} className="text-jollof-border mx-0.5" />}
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-5">
          ...
          <div className="lg:col-span-2 space-y-4 w-full max-w-3xl mx-auto lg:max-w-none">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {(Object.keys(checklist) as (keyof typeof checklist)[]).map((key) => {
                        ...
                        return (
                          <label key={key} className="flex items-center gap-3 cursor-pointer py-1 min-h-[40px]">
                            <input type="checkbox" checked={checklist[key]} onChange={(e) => setChecklist((prev) => ({ ...prev, [key]: e.target.checked }))} className="w-4 h-4 accent-jollof-orange shrink-0" />
            ...
            {step === 2 && (
              <div className="jollof-card">
                <div className="p-4 border-b border-jollof-border flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="p-4 space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    ...
                    <div className="jollof-panel p-3 text-center">
                      <div className="text-xl sm:text-2xl font-bold text-amber-400">{pkg.validationSummary.warnings}</div>
                      <div className="text-[10px] text-jollof-label">Warnings</div>
                    </div>
                    <div className="jollof-panel p-3 text-center">
                      <div className="text-xl sm:text-2xl font-bold text-green-400">{pkg.validationSummary.passed}</div>
                  ...
                  <div className="space-y-2">
                    {[
                      ...
                      { check: "Canon consistency check", status: "passed", note: "All entries consistent" },
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                      {["Story OS JSON", "PDF Report", "Art Package ZIP"].map((fmt) => (
                        <label key={fmt} className="flex items-center gap-2 jollof-panel p-3 cursor-pointer hover:border-jollof-orange/30 transition-colors min-h-[48px]">
                          <input type="checkbox" defaultChecked className="accent-jollof-orange w-4 h-4 shrink-0" />
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {["Local Download", "Google Drive", "Dropbox", "Story OS Path"].map((dest) => (
                        <label key={dest} className="flex items-center gap-2 jollof-panel p-3 cursor-pointer hover:border-jollof-orange/30 transition-colors min-h-[48px]">
                          <input type="radio" name="dest" className="accent-jollof-orange w-4 h-4 shrink-0" />
                          <span className="text-xs text-jollof-subtext">{dest}</span>
                        </label>
                      ))}
                    </div>
                  </div>
                  <Button variant="primary" icon={Download} className="w-full justify-center" onClick={handleExport} disabled={hasCritical}>
                    {hasCritical ? "Export Blocked — Fix Critical Issues" : "Export Package"}
                  </Button>
                  {hasCritical && <p className="text-xs text-red-400 text-center">Resolve critical validation issues before exporting.</p>}
                </div>
              </div>
            )}
          ...
          <div className="space-y-4 w-full max-w-xl mx-auto lg:max-w-none">
                ...
                <div className="space-y-0.5 min-w-0">
                  {pkg.packageTree.map((node, i) => (
                    <div key={i} className={`flex items-center gap-1.5 text-[11px] ${treeStatusColor(node.status)} ${node.type === "dir" ? "font-medium" : ""}`}>
                      {node.type === "dir" ? <Folder size={11} className="text-jollof-orange shrink-0" /> : <FileIcon size={11} className="shrink-0" />}
            ...
            <div className="jollof-card p-4">
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between text-xs"><span className="text-red-400">Critical</span><span className="font-bold text-red-400">{pkg.validationSummary.critical}</span></div>
                <div className="flex justify-between text-xs"><span className="text-amber-400">Warnings</span><span className="font-bold text-amber-400">{pkg.validationSummary.warnings}</span></div>
                <div className="flex justify-between text-xs"><span className="text-green-400">Passed</span><span className="font-bold text-green-400">{pkg.validationSummary.passed}</span></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <Modal open={successOpen} onClose={() => setSuccessOpen(false)} title="Export Complete" size="sm">
        <div className="text-center space-y-4">
          ...
          <Button variant="primary" icon={Download} className="w-full justify-center" onClick={() => setSuccessOpen(false)}>Done</Button>
        </div>
      </Modal>
    </AppShell>
  );
